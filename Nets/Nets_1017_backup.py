...
class MambaEncoder(nn.Module):
    def __init__(self, num_layers, mambadim, d_state=16, d_conv=4, expand=2):
        ...
        self.layer_norm = nn.LayerNorm(mambadim)
        self.GELU = nn.GELU()
...
class FeatureMamba(nn.Module):
    def __init__(self, embed_dim, mambadim,num_layers,d_state,d_conv,expand):
        ...
        self.LayerNorm = nn.LayerNorm(mambadim)
        self.LayerNorm_x= nn.LayerNorm(embed_dim)
    def forward(self, x):
        residual = x  # 残差连接
...
class TransformerRegressor(nn.Module):
    ...
    def __init__(self,  LC_num_classes,
        ...
        self.mambadim = 128
        self.LayerNorm = nn.LayerNorm(self.mambadim)
        self.LayerNorm_embedim = nn.LayerNorm(self.mambadim)
        self.LayerNorm_embedim2 = nn.LayerNorm(embed_dim)
        # 自适应位置编码器
        ...
        # 最终回归层
        self.final_layernorm = nn.LayerNorm(self.all_feat_dim*2)
        ...
        self.xRMSNorm = nn.RMSNorm(dynamic_dim)
    ...
    def forward(self, x, lat, lon, static_features):
        ...
        # 获取静态特征嵌入
        static_embed = self.static_encoder(static_features)  # [batch_size, embed_dim]
        # static_embed = static_embed.unsqueeze(1).repeat(1, self.seq_len, 1)  # [batch_size, seq_len, embed_dim]

        # 编码动态特征
        # 编码动态特征，并融合位置嵌入
        x= self.xRMSNorm(x)
        x = torch.cat([x,LC,static_features],dim=-1) # （b,1,20）
        
       
        
        dynamic_encoded = self.mamba_encoder_64(x)
        dynamic_encoded_tmp =dynamic_encoded
        dynamic_encoded = self.dynamic_embed(dynamic_encoded) #20维度-》embeddim 升维度
        
        # 交叉注意力融合位置嵌入和静态嵌入到动态特征
        # 动态变量查找位置信息
        # 算自注意力
        fused_pos_dy = self.cross_attn_all1(dynamic_encoded,pos_embed,pos_embed)
        ...
        # 融合   
        #dynamic_encoded_down = torch.cat([dynamic_encoded_down,dynamic_encoded_tmp],dim=-1) #21
        
        
        # dynamic_encoded_down = self.final_layernorm(dynamic_encoded_down)
        # 最终回归
